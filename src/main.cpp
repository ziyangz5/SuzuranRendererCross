...
void render(GLFWwindow* window, Scene* scene)
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //.............TRANSPRARENT................//
        /*
        if (savedFileCounter < maxSaveCount)
        {
            for (int glass_id = 0; glass_id < num_of_glass; glass_id++)
            {
                ...
                for (uint i = 0; i < scene->models.size(); i++)
                {
                    ...
                    if (geo_program != nullptr)
                    {
                        ...
                        geo_program->setVec3("lightPositionDelta", lightPositionDelta);
                        Model* model = scene->models[i];
                        model->ApplyTransformMatrix(glm::translate(glassesPositionDelta[glass_id]));
                        model->Draw(camera.GetViewProjectionMatrix(WinX, WinY), camera.Position, geo_program, currentFrame);
                        model->ApplyTransformMatrix(glm::inverse(glm::translate(glassesPositionDelta[glass_id])));
                        geo_program->unuse();
                    }
                }
                SaveGbufferHandler(savedFileCounter, maxSaveCount, gAttachments, "_T"+std::to_string(glass_id));
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                glClearColor(0, 0, 0, 1);
                glBindFramebuffer(GL_FRAMEBUFFER, 0);
            }
...
int main() {
    ...
    Scene* scene = SceneParser::parse_scene("../Scenes/spaceship/spaceship_opengl_area.xml");
    camera = scene->camera;
